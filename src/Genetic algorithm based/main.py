...
def hconstraint_three_courses(course_allocation):  # Hard Const 2 = Minimum 3 courses per student 
    #XXX are alll courses mapped. add one more parameter: count of courses
...
def hconstraint_teacher_samerow(schedule):  # Hard Const 6 : No teacher has duties in a row/ back to back
...
def hconstraint_course_scheduled_once(schedule):  # Hard Const 8: One course must be scheduled once only// soft. for a single sem.
...
def sconstraint_mg_before(schedule, course_allocation):  # Soft Const 4: MG courses scheduled before CS courses # skippable
    ...
    return num, bool_check
#XXX sconstraint_teacher_duty_count.  teacher, no of duties to do. 
#XXX sconstraint_teacher_slots i/p: teacher, date range for duties(or slot #).
